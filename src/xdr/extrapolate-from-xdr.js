...
const { xdr, StrKey, Keypair, Operation } = require('stellar-sdk')
module.exports = function extrapolateFromXdr(input, type) {
...
function parseNormal(anchor, object) {
  ...
  anchor.nodes = []
  const methods = _.without(Object.keys(object._attributes || {}), 'ext', 'toXDR')
  // console.log(methods)
  if (methods.length == 0) {
    debugger
  }
...
function hasChildren(object) {
  ...
  // node buffer
  if (object && _.isBuffer(object)) {
    return false
  }
  // const functions = _(object).functions()
  if (
    object &&
    typeof object.value == 'function' &&
    object.value() &&
  ...
  // int64
  if (object && _.isFunction(object.getLowBits) && _.isFunction(object.getHighBits)) {
    return false
  }
  return true
}
...
function getValue(object, name) {
  ...
  if (name === 'hint') {
    ...
    const keypair = new Keypair({ type: 'ed25519', publicKey: partialPublicKey })
    const partialPublicKeyString = `G${new Buffer(46)
      .fill('_')
      .toString()}${keypair.publicKey().substr(47, 5)}${new Buffer(4).fill('_').toString()}`
